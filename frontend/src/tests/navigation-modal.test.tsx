...
async function fillCompleteDiagnostic(
  user: ReturnType<typeof userEvent.setup>,
) {
  await user.type(screen.getByLabelText(/^Nome\s*\*?$/), 'Pessoa Teste');
...
describe('navegação e diagnóstico', () => {
  ...
  it('preserva as UTMs capturadas durante a navegação interna', async () => {
    ...
    await waitFor(() => {
      expect(
        window.sessionStorage.getItem('diagnostic_lead_utms'),
      ).toContain('google');
    });
  ...
  it('exibe todos os campos em uma única etapa sem website ou navegação', async () => {
    ...
    expect(
      screen.queryByLabelText('Site da empresa'),
    ).not.toBeInTheDocument();

    expect(
      screen.queryByText(/Etapa \d de 3/),
    ).not.toBeInTheDocument();
  ...
  it('envia o diagnóstico, normaliza o payload e mostra o agradecimento', async () => {
    ...
    const fetchSpy = vi
      ...
      .mockResolvedValue(
        new Response(
          JSON.stringify({
            message:
              'Lead de diagnóstico recebido com sucesso.',
            data: {
    ...
    renderApp(
      '/?utm_source=google&utm_medium=cpc&utm_campaign=lancamento&utm_content=banner&utm_term=ecommerce',
    );
    ...
    await waitFor(() => {
      expect(
        screen.getByText('Diagnóstico recebido'),
      ).toBeVisible();
    });
    ...
    ).not.toHaveProperty('website');

    expect(
      window.sessionStorage.getItem(
        'diagnostic_lead_utms',
      ),
    ).toBeNull();

    await user.click(
      screen.getByRole('button', {
        name: 'Conversar com nosso time',
      }),
    );

    expect(openSpy).toHaveBeenCalledWith(
      expect.stringContaining(
        'https://wa.me/5512992474969?text=',
      ),
      '_blank',
      'noopener,noreferrer',
    );
  ...
  it('exige nome, WhatsApp e e-mail corporativo antes de enviar', async () => {
    const user = userEvent.setup();
